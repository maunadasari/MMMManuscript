...
plot1A<-ggplot(df_1A,aes(x=as.factor(Year),y=citations))+
    ...
    labs(tag = "A", title="Battle Citations by Year")+
...
plot1B<-ggplot(df_1B,aes(x=fct_reorder(Publication,Citations),y=Citations))+
    ...
    coord_flip()+
    labs(tag = "B", title="Top 25 Journals Cited in Narrations")+
    theme(plot.title = element_text(hjust = 0.5,size=16),
...
plot1C<-ggplot(aes(x=Year,y=Citations),
    ...
    scale_y_continuous(breaks = pretty_breaks(n=10),expand = c(0,0),position = "left")+labs(tag="C")+
